...
import json
from .ollama_client import generate, show
import sys
...
def extract_concepts(prompt: str,
    ...
    SYS_PROMPT = (
        "Your task is extract the key concepts (and non personal entities) "
        "mentioned in the given context. "
        "Extract only the most important and atomistic concepts, if "
        "needed break the concepts down to the simpler concepts. "
        ...
        "   {\n"
        '       "entity": The Concept,\n'
        '       "importance": The contextual importance of the concept on a scale "'
...
def graph_prompt(subject: str, metadata=None, model="mistral-openorca:latest"):
    ...
    # model_info = show(model_name=model)
    ...
    # TODO Domain Kontext aus einem VS hinzufügen
    response, _ = generate(model,
                           system=SYS_PROMPT,
                           prompt=USER_PROMPT,
                           context=None,)
    try:
